...
public class ScheduleDAO {
	...
	//	dbOpenHelper = new DBOpenHelper(context, "schedules.db");
		SQLiteDatabase db = dbOpenHelper.getWritableDatabase();
		Cursor cursor = db.query("schedule", new String[]{"scheduleID","remindID","scheduleContent","scheduleDate","time","alartime"}, "scheduleID=?", new String[]{String.valueOf(scheduleID)}, null, null, null,null);
		if(cursor.moveToFirst()){
			...
			cursor.close();
			db.close();
			return new ScheduleVO(schID,remindID,scheduleContent,scheduleDate,time,alartime);
		}
		cursor.close();
		db.close();
		return null;
	...
	public ArrayList<ScheduleVO> getAllSchedule(){
		...
		//dbOpenHelper = new DBOpenHelper(context, "schedules.db");
		SQLiteDatabase db = dbOpenHelper.getWritableDatabase();
		Cursor cursor = db.query("schedule", new String[]{"scheduleID","remindID","scheduleContent","scheduleDate","time","alartime"}, null, null, null, null, "scheduleID desc",null);
		while(cursor.moveToNext()){
			...
			String time=cursor.getString(cursor.getColumnIndex("time"));
			long alartime=cursor.getLong(cursor.getColumnIndex("alartime"));
			ScheduleVO vo = new ScheduleVO(scheduleID,remindID,scheduleContent,scheduleDate,time,alartime);
			list.add(vo);
		}
		cursor.close();
		db.close();
